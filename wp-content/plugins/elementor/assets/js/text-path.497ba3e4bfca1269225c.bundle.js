...
class TextPathHandler extends elementorModules.frontend.handlers.Base {
  ...
  fetchSVG() {
    ...
    return fetch(url).then(res => res.text()).then(svg => {
      this.elements.pathContainer.innerHTML = _dompurify.default.sanitize(svg);
  ...
  setText(newText) {
    ...
    if (url) {
      ...
      newText = _dompurify.default.sanitize(newText, {
        ADD_ATTR: ['target']
      });
    }
...
//# sourceMappingURL=text-path.497ba3e4bfca1269225c.bundle.js.map